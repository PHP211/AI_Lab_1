...
int call_operator(State cur_state, State *result, int option){
	switch(option){
		...
		case 3: return pourWaterFullZ(cur_state, result);
		case 4: return pourWaterEmptyX(cur_state, result);
		case 5: return pourWaterEmptyY(cur_state, result);
		case 6: return pourWaterEmptyZ(cur_state, result);
		case 7: return pourWaterXY(cur_state, result);
		case 8: return pourWaterXZ(cur_state, result);
		case 9: return pourWaterYX(cur_state, result);
		case 10: return pourWaterYZ(cur_state, result);
		case 11: return pourWaterZX(cur_state, result);
		case 12: return pourWaterZY(cur_state, result);
		default: printf("Error Calls Operators");
			return 0;
	}
}

const char* action[] = {"First State", "Pour Water Full X", "Pour Water Full Y", "Pour Water Full Z", "Pour Water Empty X", "Pour Water Empty Y", "Pour Water Empty Z",
						"Pour Water X to Y", "Pour Water X to Z", "Pour Water Y to X", "Pour Water Y to Z", "Pour Water Z to X", "Pour Water Z to Y"};
...
void print_WaysToGetGoal(Node* node){
	...
	while(!empty_Stack(stackPrint)){
		printf("\nAction %d: %s", no_action, action[top(stackPrint)->no_function],top(stackPrint)->no_function);
